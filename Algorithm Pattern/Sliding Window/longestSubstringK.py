...
# print(longestSubstringK(2, "aabbaaccddddcd"))

#2nd solution
#O(n + n) running time and O(n) space time

def longestSubstringK2(k, s):
    distinct = {}
    maximumLength = 0
    start = 0
    maxSubstring = ''
    for i in range(len(s)):
        ...
        if (i - start + 1) > maximumLength:
            maximumLength = i - start + 1
            maxSubstring = s[start:i+1]
    
    if len(distinct) < k:
        return "Not enough substring"
    else:
        return "Max substring is " + maxSubstring + " with length " + str(maximumLength)

print(longestSubstringK2(2, "aabbaaccddddcd"))
